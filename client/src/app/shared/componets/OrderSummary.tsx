...
export default function OrderSummary() {
  ...
  const subtotal =
    basket?.items.reduce(
      (sum: number, items: Item) => sum + items.quantity * items.price,
      0
    ) ?? 0;
  const deliveryFee = subtotal > 1000 ? 0 : 500;
  const localtion = useLocation(); // Isso é usado para verificar se o usuário está na página de checkout
  ...
  return (
    ...
    >
      <Paper sx={{ mb: 2, p: 3, width: "100%", borderRadius: 3 }}>
        ...
        <Box mt={2}>
          {!localtion.pathname.includes("checkout") && ( // Mostrar o botão checkout somente se não estiver na página de finalização da compra
            <Button
              component={Link}
              to="/checkout"
              variant="contained"
